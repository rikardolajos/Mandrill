...
void Deferred::createRenderPass()
{
    ...
    colorAttachmentRefs[0] = {.attachment = 1, .layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL};
    colorAttachmentRefs[1] = {.attachment = 2, .layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL};
    colorAttachmentRefs[2] = {.attachment = 3, .layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL};

    VkAttachmentReference depthAttachmentRef = {.attachment = 4,
                                                .layout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL};

    std::array<VkAttachmentReference, 3> inputAttachmentRefs;
    inputAttachmentRefs[0] = {.attachment = 1, .layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL};
    inputAttachmentRefs[1] = {.attachment = 2, .layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL};
    inputAttachmentRefs[2] = {.attachment = 3, .layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL};
...
void Deferred::createAttachments()
{
    ...
    mPosition = std::make_unique<Image>(mpDevice, extent.width, extent.height, 1, mpDevice->getSampleCount(), format,
                                        VK_IMAGE_TILING_OPTIMAL,
                                        VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
                                        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    mNormal = std::make_unique<Image>(mpDevice, extent.width, extent.height, 1, mpDevice->getSampleCount(), format,
                                      VK_IMAGE_TILING_OPTIMAL,
                                      VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
                                      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    mAlbedo = std::make_unique<Image>(mpDevice, extent.width, extent.height, 1, mpDevice->getSampleCount(), format,
                                      VK_IMAGE_TILING_OPTIMAL,
                                      VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
                                      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
...
void Deferred::createFramebuffers()
{
    ...
    for (uint32_t i = 0; i < mFramebuffers.size(); i++) {
        std::array<VkImageView, 5> attachments = {
            imageViews[i],           mDepth->getImageView(),  mPosition->getImageView(),
            mNormal->getImageView(), mAlbedo->getImageView(),
        };

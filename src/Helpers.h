...
{
    /// <summary>
    /// Class with Vulkan helper functions. This class provides static functions that can be called directly without
    /// creating an object.
    /// </summary>
    class Helpers
    {
    public:
        /// <summary>
        /// Create a new one-time use command buffer. This call should be paired with a
        /// corresponding call to cmdEnd().
        /// </summary>
        /// <param name="pDevice">Device to use</param>
        /// <returns>Command buffer</returns>
        MANDRILL_API inline static VkCommandBuffer cmdBegin(ptr<Device> pDevice)
        {
        ...
        /// <param name="waitSemaphores">Semaphores to wait on before executing the command buffer (optional)</param>
        /// <param name="waitStages">Pipeline stages to wait on for the wait semaphores (optional)</param>
        /// <param name="signalSemaphores">Semaphores to signal once the command buffer has finished executing
        /// (optional)</param>
        MANDRILL_API inline static void cmdEnd(ptr<Device> pDevice, VkCommandBuffer cmd,
        ...
        MANDRILL_API inline static uint32_t findMemoryType(ptr<Device> pDevice, uint32_t typeFilter,
        ...
        MANDRILL_API inline static VkFormat findSupportedFormat(ptr<Device> pDevice, std::vector<VkFormat> candidates,
        ...
        MANDRILL_API inline static VkFormat findDepthFormat(ptr<Device> pDevice)
        {
        ...
        MANDRILL_API inline static void imageBarrier(VkCommandBuffer cmd, VkImage image, VkPipelineStageFlags2 srcStage,
        ...
        MANDRILL_API inline static void copyBufferToImage(VkCommandBuffer cmd, VkBuffer buffer, VkImage image, uint32_t width,
        ...
        /// <param name="height">Height of image</param>
        /// <param name="depth">Depth of image</param>
        MANDRILL_API inline static void copyImageToBuffer(VkCommandBuffer cmd, VkImage image, VkBuffer buffer, uint32_t width,
                                             uint32_t height, uint32_t depth)
        {
            VkBufferImageCopy region = {
                .bufferOffset = 0,
        ...
        MANDRILL_API inline static VkDeviceSize alignTo(VkDeviceSize value, VkDeviceSize alignment)
        {
        ...
        MANDRILL_API inline static float random(bool reset = false)
        {
